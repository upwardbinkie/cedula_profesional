    ...
    while current_cedula <= max_cedulas
        ...
        if(results.count > 0)
            ...
            if(null_count >= 50)
                ...
                diff = finish - start
                puts diff
                abort "Failed to proceed after 50 attempts"
            end
            null_count += 1
            sleep_time = (null_count * null_count)
            puts "HTTP Request returned nil for cedula #{cedula_number}. Waiting #{sleep_time} seconds to try again..."
            sleep(sleep_time)
            if(null_count == 5)
                current_cedula += 1
            end
